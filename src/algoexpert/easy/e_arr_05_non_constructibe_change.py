...
The coin value is greater than the amount of change that you can
currently create plus 1.


The coin value is smaller than or equal to the amount of change that
...
the coin that you just considered. If you're given coins [1, 2],
then you can make 1, 2, 3 cents. So if you add a coin of value
4, then you can make 4 + 1 cents,
4 + 2 cents, and 4 + 3 cents. Thus, you can make up
to 7 cents.
Optimal Space & Time ComplexityO(nlogn) time | O(1) space - where n is the number of coins
"""
...
def nonConstructibleChange_sol_1(coins):
    ...
    def can_change(target_value):
        current_sum = 0
        for current_coin_value in reversed(coins):
            print(target_value, current_coin_value, current_sum)
            if current_sum + current_coin_value <= target_value:
